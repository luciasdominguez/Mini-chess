...
public:
	Peon(const ficha& F) : t(tipo::peon), ficha(F) {	}
...
public:
	bool mover(casilla& casillas, const tablero& tablero) override;
	Torre() = default;
	//Torre(casilla* cas, ENUM_COLOR color) { posicion = cas, c = color; }
	Torre(const ficha& F) : t(tipo::torre), ficha(F) {	}
private:
...
public:
	bool mover(casilla& casillas, const tablero& tablero) override;
	Caballo(const ficha& F) : t(tipo::caballo), ficha(F) {	}
private:
	const tipo t = tipo::caballo;
};

class Alfil : public ficha {
public:
	Alfil(const ficha& F) : t(tipo::alfil), ficha(F) {	}
	bool mover(casilla& casillas, const tablero& tablero) override;
private:
...
public:
	bool mover(casilla& casillas, const tablero& tablero) override;
	Reina(const ficha& F) : t(tipo::reina), ficha(F) {	}
private:
